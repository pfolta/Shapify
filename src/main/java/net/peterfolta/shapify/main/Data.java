package net.peterfolta.shapify.main;

import javafx.scene.paint.Color;

public final class Data {

    public static final int ARTBOARD_BACKGROUND_TITLE_SIZE = 10;

    public static final double FOCUS_OUTLINE_STROKE_WIDTH = 2.0;
    public static final Color FOCUS_OUTLINE_COLOR = Color.STEELBLUE;
    public static final int RESIZE_ANCHOR_DIMENSION = 8;

    public static final int DUPLICATION_OFFSET_X = 15;
    public static final int DUPLICATION_OFFSET_Y = 15;

    private Data() {
    }

}
